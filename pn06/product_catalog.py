"""Static catalog of domestic futures products.

The stable ``product_key`` is the identity used by persistence and APIs.  The
Chinese ``display_name`` remains backward compatible with existing MarketANA
data while ``official_name`` follows the exchange product name.
"""
...
# Domestic futures listed by SHFE/INE, DCE, CZCE, GFEX and CFFEX.  Options are
# intentionally excluded because MarketANA predicts the underlying futures.
PRODUCT_CATALOG: tuple[ProductDefinition, ...] = (
    ...
    _p("CFFEX.IM", "中证1000股指", "中证1000股指期货", "CFFEX", "IM", "股指期货", "中证1000股指", "中证1000", "IM股指"),
    ...
    # Aggregate-only fallbacks.  They never fan out into concrete contracts.
